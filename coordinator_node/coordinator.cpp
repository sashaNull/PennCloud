...
/**
 * Lists the status of all servers.
 *
 * @return std::string A string containing the status of all servers in the
 * format "+OK IP:PORT#STATUS ".
 *
 * This function acquires a mutex lock to ensure thread safety while accessing
 * the list of all servers. It iterates through the list of all servers and
 * constructs a string containing the IP, port, and status of each server. The
 * status is represented as "1" for active and "0" for inactive. The constructed
 * string is returned with the "+OK " prefix and "\r\n" suffix as per the
 * protocol.
 */
string list_servers_status() {
  ...
  for (server_info *server : list_of_all_servers) {
    server_list += server->ip + ":" + to_string(server->port) + "#" +
                   (server->is_active ? to_string(1) : to_string(0)) + " ";
  }
...
/**
 * Handles the heartbeat mechanism for checking server availability.
 *
 * @param arg Unused argument (can be nullptr).
 * @return void*
 *
 * This function continuously checks the availability of servers by establishing
 * a connection to each server. It updates the status of servers based on their
 * response to the heartbeat message. Additionally, it updates the primary
 * servers and prints their details after each heartbeat check. It waits for a
 * predefined time interval (HEARTBEAT_TIME) before the next check.
 */
void *handle_heartbeat(void *arg) {
  // Continuously check server availability and update status
  while (true) {
    ...
    for (server_info *server : list_of_all_servers) {
      ...
      if (connect(sock, (struct sockaddr *)&server_addr, sizeof(server_addr)) <
          0) {
        // If connection fails, mark server as down
        cerr << "Server " << server->ip << ":" << server->port << " is down."
             << endl;
        server->is_active = false;
      } else {
        // Connection successful, send heartbeat message
        ...
        if (response.status == 2) {
          // Server response indicates it is down
          cout << "Server " << server->ip << ":" << server->port << " is down."
               << endl;
          server->is_active = false;
        } else {
          // Server response indicates it is up
          cout << "Server " << server->ip << ":" << server->port << " is up."
               << endl;
    ...
    // Update primary servers and print their details
    update_primary(range_to_primary_map, range_to_server_map,
                   map_and_list_mutex);
...
/**
 * Signal handler function to handle exit signal.
 *
 * @param sig The signal number.
 * 
 * This function prepares a shutdown message and closes the listening socket if it is open.
 * It then exits the server process with a success status.
 */
...
int main(int argc, char *argv[]) {
  ...
  // Read the parameters --> Includes the server config file location and store
  ...
  populate_list_of_servers(config_file_location, list_of_all_servers,
                           range_to_server_map);
  initialize_primaries(range_to_primary_map, range_to_server_map,
                       map_and_list_mutex);

  // Add a sigint handler
  signal(SIGINT, exit_handler);
  // Call the heartbeat thread. Assume that a function called handle_heartbeat
  // exists.
  pthread_t heartbeat_thread;
  pthread_create(&heartbeat_thread, nullptr, handle_heartbeat, nullptr);
  pthread_detach(heartbeat_thread);

  // Make a while loop that listens to new requests from the frontend servers
  // and calls a thread for each connection.
    ...
    if (strncmp(buffer, "GET ", 4) == 0) {
      ...
      if (sscanf(buffer, "GET %1023s %1023s\r\n", row_key, type) != -1) {
        ...
        if (type[0] == '\0') {
          ...
          string reqType(type);
          string range =
              get_range_from_rowname(string(row_key), range_to_server_map);
          pthread_mutex_lock(&map_and_list_mutex);
          cout << range << endl;
          string server_with_range = get_active_server_from_range(
              range_to_server_map, range, reqType, range_to_primary_map);
          pthread_mutex_unlock(&map_and_list_mutex);
          ...
          cout << "Got request: " << row_key << " Gave Response: " << response
               << endl;
        }
      ...
      if (sscanf(buffer, "PGET %1023s\r\n", tablet) != -1) {
        string range =
            get_range_from_rowname(string(tablet), range_to_server_map);
        pthread_mutex_lock(&map_and_list_mutex);
        ...
        if (primary_server != nullptr && primary_server->is_active) {
          response = "+OK " + primary_server->ip + ":" +
                     to_string(primary_server->port) + "\r\n";
        } else {
